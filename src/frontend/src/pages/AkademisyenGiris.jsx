...
const AkademisyenGiris = () => {
    ...
    const validate = () => {
        const newErrors = {};
        if (!formData.username.trim()) newErrors.username = t("error.required", "Bu alan zorunludur.");
        ...
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", backgroundColor: "var(--bg-color)" }}>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ backgroundColor: "var(--card-bg)", borderRadius: "16px", padding: "40px 36px", width: "100%", maxWidth: "400px", boxShadow: "0 2px 16px rgba(59,111,212,0.08)", display: "flex", flexDirection: "column", alignItems: "center", gap: "24px" }}>
                    ...
                    {/* İkon */}
                    <div style={{ width: "64px", height: "64px", backgroundColor: "#d4f0e4", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Users size={32} color="#2e9e6b" />
                    </div>

                    {/* Başlık */}
                    <div style={{ textAlign: "center" }}>
                        <h2 style={{ margin: 0, fontSize: "20px", fontWeight: "700", color: "var(--text-main)" }}>
                            {t("portal.academician", "Akademisyen")}
                        </h2>
                    ...
                    <form onSubmit={handleSubmit} style={{ width: "100%", display: "flex", flexDirection: "column", gap: "16px" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                            ...
                            <input
                                ...
                                style={{ padding: "10px 14px", borderRadius: "8px", border: `1.5px solid ${errors.username ? "red" : "var(--border-color)"}`, backgroundColor: "var(--bg-color)", color: "var(--text-main)", fontSize: "14px", outline: "none" }}
                            />
                            {errors.username && <span style={{ fontSize: "12px", color: "red" }}>{errors.username}</span>}
                        </div>
